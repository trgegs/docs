...
EDB Postgres Distributed (PGD) is an active/active or multi-master DBMS with configurable and often adaptable guarantees for data consistency. When used asynchronously, PGD writes to the same or related rows on multiple different nodes can result in so-called "data conflicts" when using PostgreSQL standard data types.
...
By default, conflicts are resolved at the row level. When changes from two nodes conflict, PGD picks either the local or remote tuple and the discards the other. For example, the commit timestamps might be compared for the two conflicting changes and the newer one kept. This approach ensures that all nodes converge to the same result and it establishes commit-order-like semantics for the whole cluster.

Conflict handling is configurable, as described in [Conflict resolution](#conflict-resolution). PGD can detect conflicts and handle them differently for each table using conflict triggers, as described in [Stream triggers](../striggers).

Column-level conflict detection and resolution is available with PGD, as described in [CLCD](column-level-conflicts).

By default, all conflicts are logged to the `bdr.conflict_history` table. If conflicting operations can happen against a table, (as a consequence of the way the client application does CRUD operations against the PGD cluster nodes), the owner of the table must monitor for these conflicts and analyze how to avoid them or make plans to handle them regularly as an application task. The EDB [LiveCompare](/livecompare/latest) tool is available to EDB customers to scan regularly for divergence.

Some clustering systems use distributed locking mechanisms to prevent concurrent access to data. These can perform reasonably well when servers are very close to each other in terms of latency, but can't support geographically distributed applications where very low latency is critical for acceptable performance.

Distributed locking is essentially a pessimistic approach, avoiding conflicts at all costs yet sacrificing performance. PGD advocates an optimistic approach, which is to avoid conflicts where possible but allow some types of conflicts to occur and resolve them when they arise.

## How conflicts happen

Inter-node conflicts are the result of a series of transactions that can only happen if these transactions take place concurrently on different nodes. Because the nodes transmit changes to each other only after the transactions commit, each transaction is individually valid on the node it committed on, yet isn't valid if applied on another node that did other conflicting work at the same time.

Since PGD replication essentially replays the transaction on the other nodes, the replay operation can fail if there's a conflict between a transaction being replayed and applied on the receiving node, and a transaction that was committed locally on the receiving node.

Most conflicts can't happen when all transactions run on a single node because Postgres has inter-transaction mechanisms to prevent them. Examples of these mechanisms are `UNIQUE` indexes, `SEQUENCE` operations, row and relation locking, and the `SERIALIZABLE` level of Multi-version concurrency control (MVCC) dependency tracking. All of these mechanisms are ways to communicate between ongoing transactions to prevent undesirable concurrency issues.

PGD doesn't have a distributed transaction manager or lock manager. That is part of why it performs well with latency and network partitions. As a result, transactions on different nodes execute entirely independently from each other when using the default replication setting, which is "lazy replication". Less independence between nodes can avoid conflicts altogether, which is why PGD also offers Eager Replication for when this is important.

## Types of conflict

### PRIMARY KEY or UNIQUE conflicts

The most common conflicts are row conflicts, where two operations affect a row with the same key in ways they can't on a single node. PGD can detect most of these and it applies the `update_if_newer` conflict resolver.
...
The `bdr.node_conflict_resolvers` view provides information on how conflict resolution is currently configured for all known conflict types.

#### INSERT/INSERT conflicts

The most common conflict, `INSERT`/`INSERT`, happens when `INSERT` operations on two different nodes create a tuple with the same `PRIMARY KEY` values (or if no `PRIMARY KEY` exists, the same values for a single `UNIQUE` constraint).

PGD handles this situation by keeping the most recently inserted tuple of the two according to the originating node's timestamps. (A user-defined conflict handler can override this behavior.)
...
You can effectively eliminate this type of conflict by using [global sequences](../sequences) for the primary key or the column with the unique constraint.
...
!!! Warning
    If multiple rows conflict in this way, and the resulting decision of the conflict resolution is to proceed with the insert operation, some of the data is always deleted.

Different conflict resolution behavior can be configured with a conflict trigger.
...
A simple schema provides an example that shows how this works:
...
Some UPDATE operations will succeed:
...
For applications that update primary keys, be careful to avoid runtime errors, even without PGD.
...
You can identify and resolve this situation using EDB's [LiveCompare](/livecompare/latest).
...
Both changes are applied locally, causing a divergence between the nodes. But the apply by PGD replication on the target fails on both nodes with a duplicate key-value violation error. This error causes the replication to halt and requires manual resolution.
...
As a result, we recommend strongly against allowing primary key UPDATE operations in your applications, especially with PGD. If parts of your application do change primary keys, then to avoid concurrent
...
PGD supports deferred unique constraints. If a transaction can commit on the source, then it applies cleanly on target, unless it sees conflicts. However, you can't use a deferred primary key as a REPLICA IDENTITY, limiting the use cases, just as the warning about using multiple unique constraints.
...
If the deleted row has not yet been removed by `VACUUM`, the `update_recently_deleted` conflict is generated. By default, the `UPDATE` is skipped, but you can configure the resolution for this. See [Conflict resolution](#conflict-resolution) for details.
...
When this happens, the `update_missing` conflict is generated. The default conflict resolver is `insert_or_skip`, though you can use `insert_or_error` or `skip` instead. Resolvers that do insert-or-action first try to `INSERT` a new row based on data from the `UPDATE` when possible (when the whole row was received). For reconstruction of the row to be possible, the table either needs to have
